...
const POSITIVE_WORDS = new Set(
  'good great happy love thanks amazing wonderful excellent positive yes'.split(/\s+/)
);
const NEGATIVE_WORDS = new Set(
  'bad terrible hate sad angry frustrated awful poor negative no'.split(/\s+/)
);

function scoreMessage(text: string): 'positive' | 'negative' | 'neutral' {
...
export function runSentimentPipeline(messages: string[]): SentimentResult {
